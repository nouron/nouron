...
| **Wohnhabitat** | reine Mengensteigerung (mehr Wohnkapazität/Supply-Beitrag), keine neue Fähigkeit auf keiner Stufe. **Level-Deckel korrigiert (2026-08-25):** aktuell `max_level=6` UND `max_instances=6` gleichzeitig — laut Config-Kommentar historischer Zufall, keine Design-Absicht, und GDD (Zeile 479) geht ohnehin schon von Lv1-3 pro Instanz aus. Wird auf **3 Level** gesenkt (Konsistenz mit der max.-3-Regel), Instanzen bleiben die primäre, tile-limitierte Wachstumsachse, Level wird sekundäre Feinabstimmung statt einer zweiten gleichwertigen Vollachse | Keine Beinamen, alle Stufen nackt nummeriert |
...
8. **Exklusive Ausbau-Varianten (zurückgestellt, nicht Teil dieser Spec-Version).** Owner-Idee (2026-08-23): Ausbaustufen könnten sich ab einer bestimmten Stufe in 2+ gegenseitig ausschließende Varianten verzweigen (baut man A, ist B nur mit eigener Instanz erreichbar). Game-designer-Review: Konflikt mit dem bestehenden Prinzip „Ein Pfad ist kein Ausschluss, sondern eine Sequenzierung" (GDD Zeile 670), außer die Instanz-Ausweichoption federt das wirklich ab — und die ist für die inhaltlich interessanten Kandidaten (Krankenstation, Sicherheits-Hub) heute nicht vorgesehen (nur housingComplex/harvester/hangar sind mehrinstanzenfähig, und die sind schlechte Branching-Kandidaten). Verdoppelt außerdem den Design-/Balancing-Aufwand vor jedem Playtest. **Einziger überzeugender Kandidat, falls später aufgegriffen: Krankenstation** (Stufe III trifft bereits einen harten Wirkungsdeckel — zwei Wege zum selben Ziel statt einer dritten Zahl). Hangar und Handelsposten sind explizit ungeeignet (bewusst kumulative/additive Stufen, Exklusivität würde das gerade beschlossene Design zerstören). Neu bewerten nach dem ersten Playtest der linearen 3-Stufen-Version — nur dann, wenn sich ein echtes Bedürfnis zeigt, und dann nur als Pilot an einem Gebäude (Krankenstation), nicht flächendeckend.
9. **Wohnhabitat `supply_cap`-pro-Level neu kalibrieren** (2026-08-25-Nachtrag, game-designer-Review), nachdem `max_level` von 6 auf 3 gesenkt wird — aktueller Config-Kommentar ("+48 cap") passt schon jetzt nicht sauber zur tatsächlichen `getSupplyBreakdown()`-Summierung. Zahlen-Task, kein Struktur-Thema, nach Playtest zusammen mit Punkt 3.
10. **Bauten außerhalb der Koloniezone — geprüft, verworfen (2026-08-25).** Owner-Idee: kleine Konstruktionen (z.B. Sendemast) außerhalb der Koloniezone/Hexfelder platzierbar, explizit konditional auf Lore-/Gameplay-Fit. Game-designer-Review: passt nicht in diese Umstellung. Deckt sich inhaltlich mit dem bereits bestehenden Phase-4-Konzept "Außenposten (nicht Kolonien)" (GDD Zeile 143) — keine neue Idee, sondern dessen Vorwegnahme. Die Exploration Zone ist bewusst nicht bebaubar (GDD Zeile 758: Wachstum bindet sich an die 15 Koloniefelder), das wäre ein Bruch mit einem gesetzten Design-Prinzip, kein Lückenschluss. Zusätzlich Redundanz zur Uplink-Station ("Verbindung nach außen"). Kein eigener Task hier — gehört, falls überhaupt, in eine spätere Phase-4-Außenposten-Design-Runde.
