...
- [x] **Harvester-Zweitinstanz-Bezugsquelle entworfen und freigegeben** (2026-08-05, §4c „Harvester-Zweitinstanz: Bezugsquelle"). Echter Gap war: Backend akzeptiert `instance_id=2`, aber kein Weg dorthin (`getBuildableBuildings()` schließt Harvester aus). Freigegebene Lösung: Sockel-Baseline auf **1 Instanz** umgestellt (2. Instanz = optionaler Bonus, nicht garantiert erreichbar — bewusste Owner-Entscheidung gegen Planbarkeit); Weg A = Orin (`corporate_rep`) verkauft Extraktionsrechte für 400–800 Cr bei zufälligem Erscheinen (~40–60 % Trefferquote pro Run); Weg B = Bergungsmission `mission_harvester_salvage` auf `ruin_tile`, kostenlos aber beschädigt ankommend (Reparaturpfad). **Noch offen:** volle §13.7-Neuherleitung auf die 1-Instanz-Baseline (eigener Termin), TDD-Umsetzung (`game-developer`/`backend-coder`, zwei unabhängige Stränge Weg A/Weg B), Content-Update Orins `Game Role` (`content-writer`), `ruin_tile`-Spawnrate im Code unverifiziert.
- [ ] **Pfad-C-Regolith-Hebel neu denken** — der Organika→Regolith-Tausch fällt mit der Knappheitsordnung weg (§13.7). Offen, ob Pfad C überhaupt einen großen Regolith-Hebel braucht. Hängt an der noch offenen Rollenklärung Reisender Händler vs. Cantina-Gäste (Owner-Korrektur 2026-08-04, game-designer-Vorschlag "Handelsgast-Archetyp" abgelehnt), noch nicht final entschieden.
