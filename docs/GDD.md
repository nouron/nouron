...
**Charakter-Anliegen bei zufälligen Begegnungen (Konzeptstand, nicht implementiert):** Der Begegnungspool oben deckt bislang nur den Credits-Ausgang ab (Wette/Auktion/Kontrakt). Als Erweiterung — nicht Ersatz — ist ein zweiter Begegnungstyp vorgesehen: Der Spieler trifft in der Cantina zufällig eine der etablierten Figuren, die ein konkretes Anliegen oder eine Bitte mitbringt. Der Spieler kann helfen — das kostet AP aus dem gemeinsamen Pool (§13.1), genau wie jede andere Cantina-Aktion — und erhält im Erfolgsfall eine Belohnung. Der Reiz liegt in der Bandbreite der Belohnung: Sie soll bewusst breit variieren und nicht auf eine Ressourcenachse festgelegt sein — denkbar sind Regolith, Organika oder Werkstoffe, Credits, ein AP-Bonus (nach demselben Prinzip wie der Barkeeper-Bonus, s.u.), ein Rabatt auf Baukosten oder ein zusätzliches, käufliches Schiff. Welche Figur welches Anliegen mitbringt und wie die einzelnen Belohnungsarten im Detail ausgestaltet sind, ist bewusst noch offen — hier ist nur das Grundprinzip festgehalten (Anliegen → AP-Einsatz → variable Belohnung), die volle Anliegen-Palette ist ein eigenständiger Folge-Task, ähnlich der oben skizzierten Zuordnung von Begegnungen zu Cantina-Figuren, und sollte mit demselben Task zusammen ausgearbeitet werden, um Dopplungen zu vermeiden.

**Deva & Lenn — taktische Information, wiederholbar mit Varianz (Konzeptstand, nicht implementiert):** Die beiden Figuren mit `game_role: information` (`docs/characters/veteran.md`, `docs/characters/ai_researcher.md`) bringen — anders als der Credits-Ausgang oben — keine Ressourcen-Belohnung, sondern einen taktischen Vorteil im laufenden Run mit: Deva (Militärveteranin) auf Gefahrenabwehr, Lenn (KI-Forscher) auf Erkundung. Beide sind laut Charakterbogen `occasional` platziert, also für wiederholte Begegnungen über den Run hinweg gedacht — die Begegnung ist kein Einmal-Ereignis, sondern eine wiederkehrende, bei der Dialog und Ausgang bei jedem Antreffen variieren sollen, statt immer denselben Effekt abzuspulen.
...
- **story_hook (Aldra, Sorel, die namenlose Gestalt):** Die Begegnung selbst ist bereits reine Flavor-Begegnung ohne Ressourcenwirkung — hier fällt Begegnung und Kodex-Trigger zusammen.
...
> **Scope dieser Konzeptrunde:** Trigger-Zuordnung je `game_role` und Persistenz-Prinzip sind hiermit festgelegt. Anzahl der Kodex-Einträge je Figur, die Texte selbst und die konkrete Persistenz-Implementierung (Tabelle/Schema) sind ein eigenständiger Folge-Task (`content-writer` für Text, `db-migration-agent` für Persistenz), sobald Deva/Lenn und der Kodex priorisiert werden.

**Barkeeper (Dauer-Charakter, Konzeptstand, nicht implementiert):** Anders als Corvan und die übrigen Cantina-Figuren hat der Barkeeper keinen Zufallsspawn und kein Verfügbarkeitsfenster — er ist immer in der Cantina anwesend, sobald sie gebaut ist. Statt über Angebote wirkt er über wiederholten Austausch mit dem Spieler: je häufiger der Spieler mit ihm interagiert, desto mehr Boni schaltet sich der Barkeeper frei. Der Owner-Fokus liegt auf Kenntnissen — der Barkeeper soll einer laufenden Kenntnis-Investition einen zusätzlichen AP-Beitrag hinzufügen, ähnlich einem stillen Mitspieler, der ergänzend zum Analytiker in dieselbe Kenntnis einzahlt, ohne selbst eine Kenntnis freizuschalten. Damit wäre der Barkeeper kein weiterer Handelskanal, sondern ein eigenständiger, dauerhaft verfügbarer AP-Hebel für den Kenntnis-Pfad. Als späterer UI-Ausblick (kein Teil dieser Konzeptrunde): Investierte AP pro Kenntnis sollen im Frontend sichtbar aufschlüsseln, welcher Anteil vom Barkeeper stammt (eigenes Symbol/Badge neben der Balken-/Zahlenanzeige), damit sein Beitrag vom Analytiker-Beitrag unterscheidbar bleibt.
