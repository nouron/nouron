...
| `buildings.php` → `build_cost` (Regolith-Anteil), Hangar (id 44) | **120 → 95** | entschieden 2026-08-11, G4-Audit unten — Implementierung noch offen (separater Schritt, s. dort) |
...
2. **G4** — entschieden (2026-08-11, Option 3): Hangar 120→95 gesenkt, alle drei Pfadgebäude jetzt einheitlich 95 Rg; bioFacility bleibt bei 70 als dokumentierte Ausnahme (kein Pfadgebäude). Details und Begründung siehe Audit-Block oben. **Code-Änderung (`config/buildings.php`, `build_cost` für Hangar id 44) ist ein separater Implementierungsschritt**, hier nicht ausgeführt — inkl. CHANGELOG-Eintrag, Prüfung/Anpassung von `app/Console/Commands/ResetPlayer.php` (Szenario-Kommentare rechnen z. T. mit dem alten Hangar-Preis) und der 835-Rg-Zielkolonie-Bedarfskette (Punkt 1–4 der Neuherleitung oben, s. Folgepunkt unten).
