...
> ⚠️ BALANCE CONCERN (Config-Review-TODO): Der Wechsel von „1 Gebäude" auf „alle Gebäude" verändert die Gesamt-SP-Attrition eines einzelnen Sturm-Ereignisses grundlegend, ohne dass sich `game.encounter.storm.chance_per_building`, `damaged_sp_loss_pct` oder `critical_threshold_pct` bislang geändert hätten — diese Werte wurden gegen „ein Treffer, ein Gebäude" kalibriert (siehe Regolith-Reserve-Herleitung, GDD §18, Nachtrag 2026-08-16: ein einzelner Kritisch-Treffer kostete dort im Schnitt ~77,5 Rg für *ein* Gebäude). Mit koloniweitem Wirkbereich potenziert sich dieser Wert näherungsweise mit der Zahl betroffener Gebäude, was die dortige Sol-30-Deadline-Rechnung ungültig macht. Vor der Umsetzung sollte der Config-Owner (game-developer in Rücksprache mit game-designer) prüfen, ob `chance_per_building`/`damaged_sp_loss_pct` abgesenkt werden müssen, sobald der Scope-Wechsel im Code ankommt, und die Regolith-Reserve-Herleitung in §18 danach neu rechnen. Keine neuen Zahlen hier im GDD-Text (ADR 0004) — nur der Rechenweg-Verweis, damit die Neukalibrierung nicht vergessen wird.
...
- **Umsetzungslücke Sturm-Wirkbereich (2026-09-03):** `EncounterService`/`GameTick` treffen Sturm im Code noch als Einzelgebäude-Ereignis (1 zufälliges Gebäude) — die oben spezifizierte koloniweite Zielwahl (alle Gebäude außer Harvester, je eigene Ausgangsstufe, ein gemeinsamer Trust-Ausgang, siehe „Wirkbereich"-Abschnitt) ist noch nicht implementiert. Reine Umsetzungsaufgabe, keine offene Design-Frage mehr — die vormals hier offene Frage nach dem Sturm-Zielumfang ("1, selten 2 zufällige Gebäude" vs. größerem Wirkbereich) ist mit der Owner-Entscheidung 2026-09-03 abschließend zugunsten von koloniweit entschieden.
