...
| 2 | `hint_repair` | Irgendein Gebäude unter Maximal-Statuspunkten (`status_points < max_status_points`) | "Deine Startgebäude sind beschädigt — tippe ein Gebäude an und nutze „Reparieren" (1 Bau-AP), um die Substanz zu sichern." | Colony-Screen | — |
...
> **Designentscheidung zu Rang 2 (Reparieren):** Alle drei Startgebäude starten auf `status_points=16/20` (80% — beschädigt aber funktionsfähig). Reparieren ist die billigste (1 Bau-AP/Klick), sofort belohnende erste Aktion und lehrt den Bau-AP-Loop, bevor teurere Entscheidungen (Harvester verlegen, CC-Ausbau) anstehen. Kein Tick-Gate — der Hint erscheint ab Sol 1 und löst sich organisch auf, sobald alle Gebäude voll repariert sind. Steht hinter `hint_1` (Baumeister liefert erst die nötigen Bau-AP), aber vor allen teureren Aktionen.
