                ...
                <div class="tech-card tech-{{ $tech['type'] }} status-{{ $tech['status'] }}"
                    ...
                    <span class="tech-status-chip chip-{{ $tech['status'] }}">
                        @if($tech['type'] === 'personell')
                            @if($tech['status'] === 'built'){{ __('techtree.advisor_hired') }}
                            @elseif($tech['status'] === 'available'){{ __('techtree.advisor_available') }}
                            @else{{ __('techtree.advisor_locked') }}
                            @endif
                        @elseif($tech['type'] === 'ship')
                            @if($tech['status'] === 'built'){{ $tech['level'] }}{{ $tech['hangar_cap'] ? ' / '.$tech['hangar_cap'] : '' }}
                            @elseif($tech['status'] === 'available'){{ __('techtree.status_available') }}
                            @else{{ __('techtree.status_locked') }}
                            @endif
                        @elseif($tech['is_instanced'] ?? false)
                            @if($tech['max_level'] === 1)
                                @if($tech['instance_count'] > 0){{ __('techtree.advisor_placed') }}
                                @else{{ __('techtree.advisor_not_placed') }}
                                @endif
                            @elseif($tech['status'] === 'built'){{ $tech['instance_count'] }}@if($tech['max_level']) / {{ $tech['max_level'] }}@endif
                            @elseif($tech['status'] === 'available'){{ __('techtree.status_available') }}
                            @else{{ __('techtree.status_locked') }}
                            @endif
                        @else
                        ...
                        <div>
                            ...
                            <template x-if="selectedTech.is_instanced && selectedTech.instance_count > 0">
                                <div class="detail-row">
                                    ...
                                    <span x-text="selectedTech.instance_count + (selectedTech.max_level ? ' / ' + selectedTech.max_level : '')"></span>
                                </div>
                            </template>
                            <template x-if="selectedTech.required_desc">
                                <div class="detail-row">
                        ...
                        <div>
                            <template x-if="selectedTech.level > 0">
                                <div class="detail-row">
                                    ...
                                    <span x-text="selectedTech.level + (selectedTech.hangar_cap ? ' / ' + selectedTech.hangar_cap : '')"></span>
                                </div>
                            </template>
                            <template x-if="selectedTech.required_desc">
                                <div class="detail-row">
