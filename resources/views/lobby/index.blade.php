...
@section("content")
    <div class="lobby">
        ...
        {{-- ── Page header ──────────────────────────────────────────────────────── --}}
        <hgroup class="lobby-page-header">
            <h1 class="lobby-page-title">{{ __("lobby.page_title") }}</h1>
            <p class="lobby-page-subtitle">
            ...
            <div class="lobby-run-grid">
                @foreach ($active as $run)
                    ...
                    <article class="lobby-run-card">
                        ...
                        <progress value="{{ $displayTick }}" max="{{ $tickLimit }}"
                            title="{{ $solPct }}%"></progress>
                        ...
                        <footer>
                            ...
                            <form method="POST" action="{{ route("lobby.abandon", $run->id) }}" class="lobby-inline-form"
                                onsubmit="return confirm(@json(__("lobby.abandon_confirm")))">
                                @csrf
                                <button type="submit" class="secondary outline lobby-btn-danger-outline">
            ...
            <div class="lobby-run-grid">
                @foreach ($pending as $run)
                    @php
                        ...
                        $bypass = is_array($run->settings["bypass"] ?? null) ? $run->settings["bypass"] : [];
                    ...
                    <article class="lobby-run-card">
                        ...
                        <form method="POST" action="{{ route("colony.rename") }}" class="lobby-rename-form">
                            @csrf
                            @method("PATCH")
                            <input type="text" name="name" value="{{ old("name", $colonyName) }}" minlength="2"
                                maxlength="50" required aria-label="{{ __("lobby.rename_label") }}">
                        ...
                        <footer>
                            ...
                            <form method="POST" action="{{ route("lobby.start") }}" class="lobby-inline-form">
                                @csrf
        ...
        <h2 class="lobby-section-heading lobby-highscore-heading">
            {{ __("lobby.highscore_title") }}
        </h2>

        @if ($finishedRuns->isNotEmpty())
            <div class="lobby-highscore-scroll">
                            ...
                            <tr>
                                <td>
                                    ...
                                    @if ($entry["ended_at"])
                                        <br>
                                        <small
                                            class="lobby-hs-meta">{{ __("lobby.highscore_ended", ["date" => $endedDate]) }}</small>
                                    @endif
                                </td>
                                <td>
                                    <span class="lobby-status-pill lobby-status-pill--{{ $statusCls }}">
