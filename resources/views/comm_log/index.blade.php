...
@section("content")
    <div class="comm-log-page">
        ...
        <nav class="comm-tabs" aria-label="{{ __("comm_log.page_title") }}">
            <a href="{{ route("comm.log") }}" class="comm-tab @if ($tab === "log") comm-tab-active @endif">
                <i class="bi bi-journal-text"></i>
                {{ __("comm_log.tab_log") }}
            </a>
            <a href="{{ route("comm.nexus") }}" class="comm-tab @if ($tab === "nexus") comm-tab-active @endif">
            ...
            @foreach ($entries as $entry)
                ...
                @if ($tab === "nexus")
                    ...
                    @php
                        ...
                        $badgeClass = $badgeText
                            ? "comm-nexus-badge comm-nexus-badge--" . Str::slug($badgeText)
                            : "comm-nexus-badge comm-nexus-badge--neutral";
                    @endphp
                ...
                @else
                    ...
                    <div class="comm-entry">
                        ...
                        @if (($entry["_collapsed"] ?? 0) > 1)
                            <span class="comm-entry-collapsed">×{{ $entry["_collapsed"] }}</span>
                        @endif
                    </div>
                @endif
            @endforeach
