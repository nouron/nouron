...
- Feat (in Arbeit, Branch `feat/hangar-mission-success-chance`): Hangar-Missionen bekommen Erfolgschance + wählbaren Schwierigkeitsgrad (leicht/normal/schwer) statt Garantie-Reward — Plan mit 12 Tasks per Subagent-Driven Development, Tasks 1-10 fertig+reviewed (Migration, Config, `successChanceFor()`, Dispatch-Validierung, Tick-Resolution mit Erfolgs-Roll/Reward-Skalierung/Hard-Fail-Wear, Sol-Report, Lang-Strings, Katalog-Preview), Task 11 (Alpine-Dispatch-Dialog UI) angefangen, Task 12 (GDD/game-reference) offen. Session für heute unterbrochen, morgen Fortsetzung.
