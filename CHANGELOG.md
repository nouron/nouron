...
**Fix: Post-Phase-1-Credit-Ökonomie-Kollaps behoben** (`config/game.php`, `app/Services/RunProgressService.php`). PlaytestBot zeigte Credits dauerhaft bei 0 ab Sol ~40 (55 von 95 Solen, 58% des Runs) — Ursache: der Rang-2→3-Berater-Upkeep-Sprung (30→80, 2,67×) blieb beim 07-19-Fix fast unangetastet, und beide Zusatzeinkommen (Handelsvertrag, Corvan-Handel) hängen an einer gebauten Cantina, die der Bot (und potenziell Spieler mit Sciencelab/Hangar-first-Pfadwahl) oft gar nicht baut. Vier Hebel (GDD §18.4 Nachtrag 2026-08-14): `advisor.upkeep` [1=10,2=30,3=80]→[1=10,2=25,3=50], `credits.nexus_subsidy` 30→50, `credits.relay_bonus_per_uplink_level` 20→35, `advisor.promotion_costs[3]` 400→250. `task_credit_reserve`-Schwelle zusätzlich von hartcodiert 5000 nach `config('game.run.task_credit_reserve_threshold')`=3000 ausgelagert (war unter dem alten Kollaps faktisch unerreichbar). Ergebnis: permanenter 0-Kollaps behoben, Credits pendeln jetzt niedrig statt festzukleben — `task_credit_reserve` selbst wird vom Bot trotzdem nicht erreicht, weil er jeden Überschuss sofort verausgabt statt zu sparen (Bot-Spielstil, kein Ökonomie-Fehler mehr, offener Folgepunkt). Suite grün (982 Tests, 0 Skips).
...
**Iteration 2: Startbestand 300→340 + zwei Pfadgebäude-Levelup-Bugs im PlaytestBot behoben.** Owner-Vorgabe verschärft: Phase 2 muss unter Sol 25 erreichbar sein, bei sehr gutem Run unter Sol 20. Mit Startbestand 300 kam Berater 2 real erst Sol 23 statt der von der Floor-Formel vorhergesagten ≈Sol 15,4 — Ursache gefunden (GDD §13.7 Nachtrag 2026-08-13): Bedarfskette war um 35 Rg unterzählt (`ColonyController::placeBuilding()` setzt neue Gebäude auf `level:0`, der 0→1-Sprung ist ein separater, mit 25 Rg bepreister Schritt — für beide Pfadgebäude komplett übersehen), korrigierte Summe 535 statt 500 Rg. Zusätzlich ein Bot-Bug: `cheapestPendingPathBuildingCost()` behandelte ein platziertes-aber-ungelevelt es Pfadgebäude als "fertig" und gab den Rg-Puffer vorzeitig frei, `productionInvestCandidate()` verspielte den Regolith dann an ein unabhängiges Gebäude statt am günstigen 25-Rg-Levelup, der den Beraterslot freischaltet — beide Stellen gefixt (Puffer berücksichtigt Levelup-Bedarf, Invest-Ordering bevorzugt Pfadgebäude auf Level 0). Neuer Startbestand 340 (Floor ≈Sol 15,1). Ergebnis über 3 Seeds: `phase2_start_sol`=20-22 durchgehend (vorher: nie erreicht). Suite grün, 980 Tests, 0 Skips (auch der bisher wegen Tile-Nichtdeterminismus geskippte Determinismus-Test läuft jetzt durch).

## 2026-08-12

**PlaytestBot: Harvester-Zweitinstanz-Pfade (Weg A/B) implementiert** (`tests/Feature/Playtest/BotStrategy.php`). Der Bot kannte die eigentlich vorgesehene Bezugsquelle für die 2. Harvester-Instanz (GDD §4c, `HarvesterEntitlementService`) bisher nicht — nur die 1-Instanz-Baseline mit Explore/Relocate. Vier neue Regeln: `buy_corporate_harvester_offer` (Weg A, Orins Angebot kaufen sobald verfügbar/bezahlbar), `place_harvester_instance2` (verdiente Instanz auf freier Regolith-Tile platzieren), `deep_scan_signal_tile` (Voraussetzung für `event_ruin`-Tiles — der Bot hat vorher nie deep-gescannt, Event-/Missionsinhalte waren dadurch strukturell unerreichbar), `dispatch_salvage_mission` (Weg B, Bergungsmission mit Frachter/Korvette). Ein echter Bug beim Bauen gefunden und gefixt: `place_harvester_instance2` prüfte nicht, ob die Zieltile schon bebaut war (u. a. vom 1. Harvester selbst) — führte zu mehreren Sol-Fehlversuchen (`tile_occupied`) bevor zufällig eine freie Tile getroffen wurde; jetzt mit `whereNotExists`-Occupied-Check wie bei der regulären Gebäudeplatzierung. Ergebnis über mehrere Seeds/Reruns: `phase2_start_sol` wird jetzt in den meisten Läufen erreicht (vorher nur vereinzelt) — Runs scheitern weiterhin an `time_limit` (zu wenig Sole für Phase 2 übrig), das bleibt ein offener, nachgelagerter Befund. Volle Suite grün (968 Tests, 2 vorbestehende Skips).
...
**GDD-Balance-Audit G4 gelöst: Hangar-Preis 120→95 Rg** (`config/buildings.php`). Die Leitplanke "jede Errichtungsstufe kostet 5-8 Sole Ansparzeit" (GDD G4) traf bei keinem Einkommensniveau auf alle drei Pfadgebäude gleichzeitig zu — Hangar (120 Rg) brauchte beim Zyklusmittel (12,9 Rg/Sol) 9,3 Sole, über der Obergrenze. Lösung: statt die drei Preise einzeln zu justieren, alle drei Pfadgebäude (Sciencelab/Hangar/Bar) auf einheitlich 95 Rg gebracht — passt bei Frischwert (18 Rg/Sol → 5,3 Sole) und Zyklusmittel (12,9 → 7,4 Sole) gleichzeitig, deckt sich mit der bestehenden §4b-Paritäts-Anforderung. bioFacility/Agrardom (70 Rg) bleibt unverändert — kein Pfadgebäude, dokumentierte Ausnahme (Pflichtbau vor CC Lv2). `BuildResourceSinkTest::test_place_hangar_deducts_regolith` + Kommentare in `config/buildings.php`/`BotStrategy.php` nachgezogen, volle Suite grün, keine Regressionen (PlaytestBot-Tests skippen fallweise nichtdeterministisch, seed-4242-Flake, bereits vor diesem Fix auf master vorhanden). Kollateraleffekt mitgezogen: Hangar wäre bei gleichem Baupreis sonst auch bei `supply_cost` (4 vs. 6/8) der günstigste der drei Pfade gewesen — auf Owner-Wunsch provisorisch vereinheitlicht: alle drei Pfadgebäude jetzt `supply_cost=6` (Sciencelab 8→6, Hangar 4→6, Bar unverändert). `game:sync-config` in der Dev-DB ausgeführt — dabei aufgefallen: Dev-DB war seit dem 08-03-Rebalancing komplett nicht gesynct (decay_rate/build_cost mehrerer weiterer Gebäude+Schiffe veraltet), mit synchronisiert (20 Zeilen, unabhängig von G4). Offen (nicht Teil dieses Fixes): `ResetPlayer.php`-Kommentare referenzieren bereits vor diesem Fix eine veraltete 80-Rg-Zahl, G6/G2-Zielkolonie-Bedarfskette leicht verschoben (Richtung bekannt, nicht neu hergerechnet), GDD-Richtwert-Ranges Z.401 veraltet.

**PlaytestBot: Deadlock durch `exploreCandidate()`-Ring-Cap behoben** (`tests/Feature/Playtest/BotStrategy.php`). Der Bot explorierte hartcodiert nur `ring<=2` — sobald die Regolith-Tiles dort erschöpft waren, gab es weder etwas zu explorieren noch ein Umsiedlungsziel für den Harvester (`harvesterRelocateCandidate()` bietet nur bereits explorierte Regolith-Tiles an). Seed 4242 fror dadurch ab Sol 20 komplett ein (Regolith konstant bei 25, 14-16 AP ungenutzt bis Sol 95). `config('game.colony.explore_cost_per_ring')` sieht Ring 3 explizit vor (3 AP) — der Cap war ein reines Bot-Test-Artefakt, keine Engine-Grenze. Fix: Ring-Cap entfernt. Ergebnis: Bot erreicht jetzt `phase2_start_sol=75` (vorher nie), 491/678 statt 187/198 Actions. Der Run scheitert danach weiterhin an `time_limit` (nur 20 Sol für Phase 2) — separater, nachgelagerter Befund, nicht Teil dieses Fixes. Offen: Bot hat keine Logik für die eigentlich vorgesehene 2. Harvester-Instanz via corporate_rep/Bergungsmission (siehe Eintrag 2026-08-05) — geplante Folgearbeit.

## 2026-08-10

**AP-Pool zusammengelegt** (ROADMAP Phase 3o, Stufe 2, GDD §13.1). Die fünf getrennten AP-Domänen (`construction`/`research`/`navigation`/`economy`/`strategy`) sind zu einem gemeinsamen Kolonie-Pool zusammengelegt — jeder Berater zahlt unabhängig von seiner Domäne in denselben Pool ein, die Domäne bestimmt künftig nur noch einen Effizienzbonus (§13.3, noch nicht implementiert). `AdvisorService::getAvailableActionPoints()`/`getTotalActionPoints()`/`lockActionPoints()` verlieren ihren `$type`-Parameter; die vier Domänen-Getter (`getConstructionPoints` etc.) entfallen. `ap.base` 6→12, `advisor.ap_per_rank` [4,7,12]→[2,3,4] (GDD §13.6). `strategist`-Beratertyp entfernt (zurückgestellt, GDD-Entscheidung 2026-08-02), `advisor.max_slots` 5→4. Resourcebar zeigt einen AP-Chip statt fünf. `PersonellService` in `AdvisorService` umbenannt und von `app/Services/Techtree/` nach `app/Services/` verschoben (kein Techtree-spezifischer Service mehr).

**AP-Pool-Zusammenlegung: Whole-Branch-Review-Fixrunde.** Vor PR-Erstellung fielen bei einem Review über alle 10 Umsetzungs-Tasks hinweg Lücken auf, die kein Einzel-Task-Review sehen konnte: `sol-button.blade.php` (vom Task-Zuschnitt keinem Task zugeordnet) las nach der Konsolidierung noch die alten Domänen-Keys aus `GET /sol/remaining-ap` und zeigte im "Sol beenden?"-Dialog eine leere AP-Zusammenfassung — behoben, dazu ein Feature-Test für die bis dahin ungetestete Route ergänzt. Weitere Funde: Berater-UI (`advisors.js`, `advisors/index.blade.php`, `command_center.blade.php`) zeigte noch Domänen-Labels ("Bau-AP"/"Forschungs-AP"/…) trotz gemeinsamen Pools, jetzt neutrales `"+N AP"`; `.claude/agents/game-developer.md` dokumentierte noch die alte `PersonellService`-Signatur; eine Testassertion war gegen `ap.base=12` unfalsifizierbar geworden. Dazu diverse Aufräumarbeiten: strategist-Leiche im `ResetPlayer`-Debug-Szenario, stale Config-Fallback-Zahlen (`ap_per_rank`, `max_slots`), veraltete Docblock-Kommentare, unbenutzte `RunReport::AP_TYPES`-Konstante und drei wortidentische AP-Wrapper-Methoden im Playtest-Bot konsolidiert. Volle Suite (970 Tests) + Pint grün.

**Folgefix: `task_senior_advisors`-Missionsziel nach `max_slots` 5→4 unerreichbar.** Die AP-Pool-Zusammenlegung (PR #240) hatte `advisor.max_slots` von 5 auf 4 gesenkt (`strategist` zurückgestellt), aber `RunProgressService::updateSeniorAdvisors()` prüfte weiterhin hartcodiert `$totalAdvisors >= 5` — das Run-Missionsziel "Expertenstab" war dadurch im echten Spiel nie mehr erfüllbar. Referenziert jetzt `config('game.advisor.max_slots')` statt die Zahl zu duplizieren (TDD: Test zuerst rot, dann Fix). GDD entsprechend nachgezogen: Kenntnisse-Matrix 7×5→7×4, "Max. aktive Sekundäreffekte" 5→4 (7 Kenntnisse/4 Slots → 3 ohne Sekundäreffekt statt 2, mit neuem Balance-Hinweis zur schwächeren Wirkung auf Run-Ebene), Missionsziel-Katalogtext ohne feste Zahl. UI-Text (`lang/de`+`lang/en`/`run.php`) ebenfalls ohne feste Zahl formuliert, bleibt so bei künftigen `max_slots`-Änderungen korrekt.
