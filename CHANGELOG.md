...
- Fix: A37 — `BotStrategy::researchCandidate()` blieb permanent an einer einzigen, `knowledge_cc_level_cap`-gegateten Kenntnis hängen (Root Cause: `levelup`-Fehler `knowledge_cc_gate` fällt nicht auf den `add`-Fallback zurück, und die Kandidatenwahl kannte den Gate nicht) — überspringt jetzt CC-gegatete Kenntnisse. Ergebnis: 6 von 7 Kenntnissen erreichen jetzt Lv4 (statt 1 von 7), ein Phase-2-Ziel erstmals erfüllbar. `task_research_lead` bleibt offen — neuer Blocker ist CC-Lv5, kein Bot-Bug mehr. Siehe ROADMAP A37.
