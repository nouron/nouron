...
- Fix: A44 Schritt 1 — Zufallszahlen für Karte, Begegnungen, Missionen, Cantina und Corvan kommen aus einem sauber geseedeten Generator (`SeededRandom`). Vorher gab es bei echten Spielern nur wenige Dutzend Kartenvarianten, Begegnungen lösten zu oft aus und Missionen gelangen fast immer.
- Fix: T20 — Test-/Bot-Datenbank übernimmt Gebäude- und Schiffswerte jetzt immer aus der Config (`game:sync-config` im TestSeeder); vorher liefen Tests und Bot-Batches mit veralteten, deutlich zu niedrigen Verfallsraten.
...
- Feature: A41 — Charakter-Anliegen bei zufälligen Cantina-Begegnungen implementiert. 9 Figuren (6 bar_trade + bewusster Leitplanken-Bruch: auch die 3 story_hook-Figuren Aldra/Sorel/Stranger) bekommen je ein Anliegen mit AP-Kosten und variabler Belohnung (Zusatzschiff, Baukosten-Rabatt-Gutschein, AP-Bonus, Werkstoffe, Regolith, Credits, Trust über neue Event-Keys `story_concern_resolved`/`story_concern_failed`). Läuft als zweiter, unabhängiger, selteneren Roll NACH dem bestehenden Credits-Begegnungspool (max. 1 Cantina-Sonderbegegnung/Sol über beide Pools). TDD, volle Suite grün.
- Feature: A42 — Deva/Lenn Vier-Ausgänge-Pool implementiert (`BarService::generateInformationEncounterForColony()`/`resolveInformationEncounter()`, `bar_information_encounters`, `colony_information_pool_state`): eigener dritter Cantina-Kanal, flacher Spawn-Chance/Tick, unabhängig vom Encounter-/Concern-Slot. Deva (Drill-Reaktionspuffer halbiert nächste Instabilität/Seuche-Konsequenz, in `GameTick::rollInstability()`/`rollPlague()` verdrahtet; wahlweise Kenntnis-Boost Geologie/Gesundheit) und Lenn (100%-Navigation-Rabatt für die nächste `exploreTile()`-Aktion; Kenntnis-Boost Kartografie) — je 2 mechanische Ausgänge 1×/Run gedeckelt, 2 narrative unbegrenzt. Charakter-Kodex (`CharacterCodexService`, `character_codex_entries`, user-persistent über Runs hinweg, eigener Screen `/user/codex`) für den ganzen **15**-Figuren-Cast eingehängt (frühere "14"-Zählung übersah Voss/scrap_dealer durchgängig): information- (jede Begegnung), bar_trade- (personalisierte Flavor-Zeile beim Tausch, `acceptOffer()` neuer optionaler `characterSlug`-Parameter), dedicated- (Zara/Voss/Orin), story_hook- (`pickStoryEncounter()` + Concern für Aldra/Sorel/Stranger) und permanent-Trigger (Tomas, Cantina-Ausbaustufe 1-5). TDD durchgängig, echte Browser-Verifikation via Playwright. Nebenbefund: bereits bestehender Quote-Kollisions-Bug im A36-Angebots-Annehmen-Button (`bar.blade.php`, doppelte Anführungszeichen in `@click`-Attribut kollidierten mit `@json()`) gefunden und gefixt — brach die UI für jedes Angebot mit zugeordnetem Charakter. `docs/game-reference.md` ohne Änderungsbedarf (keine bestehende Zahlen-Tabelle betroffen).
...
- Fix: A37-Rest — Werkstoff-Engpass-Untersuchung: der Bot nutzte keine der vorhandenen Werkstoff-Quellen (dritter Zero-Rule-Bug der A34-Familie). Neue `dispatch_compounds_mission`-Regel (`mission_salvage_sweep`). Dabei entdeckt: `shipToRequest()` kaufte bei unleistbarem Frachter immer eine weitere Drohne statt zu sparen — 3 Test-Seeds endeten mit 5-7 Drohnen und null Frachtern. Drohnen-Kauf auf 2 gedeckelt. Neues Schiffs-Tracking in `RunReport` (`RunReportShipSnapshotTest`). Ergebnis: Frachter werden jetzt erworben, Compounds-Missionen feuern erfolgreich — Trust-Gebäude werden trotzdem weiterhin nie gebaut, eigene Folge-Analyse nötig (Compounds-Bestand-Instrumentierung fehlt noch).
...
- Fix: A38 — `game:playtest --seeds=X` reproduzierte nichts außer der Objective-Ziehung, weil `ColonyTileService` die komplette Sol-1-Startkarte mit ungeseedetem `random_int()`/`shuffle()`/`array_rand()` würfelte (bewusste Roguelike-Variation, aber `rng_seed` wurde zudem erst NACH der Kartengenerierung gesetzt). Ein `$rngSeed` treibt jetzt Karte und `runs.rng_seed` gemeinsam, optional durchreichbar (`resetColonyToSol1(..., rngSeed:)`) — echte Spieler unverändert zufällig. TDD: `ColonyTileServiceSeededGenerationTest`. Verifiziert: zwei Solo-Läufe von `--seeds=1001` liefern jetzt byte-identische Reports.
- Test: A37-Teilbefund — `RunReport` trackt jetzt Kenntnis-Level pro Sol (vorher gar nicht erfasst). Ein voller Bot-Lauf zeigte: nur 1 von 7 Kenntnissen je angefasst, `task_research_lead` (3 Kenntnisse auf Lv5) strukturell unerreichbar, trotz 63% täglich verfallener AP.
- Fix: A37 — `BotStrategy::researchCandidate()` blieb permanent an einer einzigen, `knowledge_cc_level_cap`-gegateten Kenntnis hängen (Root Cause: `levelup`-Fehler `knowledge_cc_gate` fällt nicht auf den `add`-Fallback zurück, und die Kandidatenwahl kannte den Gate nicht) — überspringt jetzt CC-gegatete Kenntnisse. `Playtest.php`-Timeout 240→400 (Bot handelt jetzt deutlich mehr AP erfolgreich ab). 8-Seed-Batch nach Fix: 1 von 8 Runs gewinnt jetzt (vorher 0 von 8), 5 von 8 erreichen mindestens 1 Ziel (vorher meist 0). `task_research_lead` bleibt offen — neuer Blocker ist CC-Lv5, kein Bot-Bug mehr. Siehe ROADMAP A37.
- Balance: A39 — Sciencelab-Kenntnis-Rabatt verstärkt und auf Lv2 vorgezogen (`sciencelab.knowledge_ap_cost_reduction_per_lv`: `[4=>3,5=>2]` → `[2=>6,3=>7,4=>8,5=>9]`, game-designer-kalibriert). TDD-Anpassungen in `ResearchServiceKnowledgeDiscountTest`/`ProjectBonusServiceTest`/`HangarMissionResolutionTest`, `docs/game-reference.md` nachgezogen. 8-Seed-Batch: 3 von 8 Runs gewinnen jetzt (vorher 1 von 8), `task_research_lead` erfüllt sich in 2 der 3 gewonnenen Runs.
...
- Feature: A35 — Cantina-Begegnungspool, mechanisches Grundgerüst (Ersatz-Hebel für den gestrichenen Konsul-Handelsvertrag, F3/§12). Neue Tabelle `bar_encounters`; ein gemeinsamer Ereignis-Slot, ein Roll (`BarService::generateEncounterForColony()`, skaliert über Bar-Level statt Konsul-Rang) entscheidet zwischen drei Ausgängen: Wette (Zara, chance-basiert, Totalverlust bei Fehlschlag), Auktion (Voss, garantierte Einmalzahlung), Kurzzeit-Kontrakt (generisch, Credits/Sol über ein festes Fenster, `GameTick::processBarEncounters()`). Neuer Endpoint `POST /bar/accept-encounter/{encounter}`, neue Hotspot+Dialog-UI im Cantina-Screen. TDD (`BarEncounterTest`, `GameTickBarEncounterTest`, `BarEncounterControllerTest`), echte Browser-Verifikation via Playwright. Werte sind ein erster Entwurf (keine kalibrierte Balance, analog A34). A36 (volle Charakter-Zuordnung übriger Cantina-Figuren) bleibt eigener Folge-Task.
...
- Docs: Audit-Kategorien B (implementiert, aber als offen geführt) und D (Doku-Hygiene) behoben — GDD §18.6-Tabelle auf die echten Restpunkte reduziert, §9-Sturm-Status/§15-Kombo-Blacklist/§4c-`knowledge_cc_level_cap`-Aussage korrigiert, ROADMAP (Stand 2026-09-06: Harvester-Erschöpfung, Level-Deckel, Almanach/NexusDB, 8 statt 9 Objectives, obsolete Phase-2-Prio-4-Punkte, Stratege/Systemkarte-Reste), CLAUDE.md-„Aktueller Stand" (entfernte Flotten-/Systemkarten-/INNN-Features raus, implementierte Systeme rein), `game-reference.md`-Gebäudetabelle auf das Ausbaustufen-System (Lagerhalle raus), `gdd-balance-checklist.md` A.1/A.4 abgehakt, `handoff-ap-ratenmodell.md`/`entity-chips.md` Status-Header, `onboarding.md` Hint-Texte auf „AP", drei irreführende Config-Kommentare in `config/game.php` (`knowledge_cc_level_cap` ist durchgesetzt, Housing-Cap summiert Level, `nexus_milestones` wird nicht gelesen).
- Docs: GDD von gestapelten Nachträgen auf Ist-/Soll-Zustand konsolidiert (Owner-Auftrag) — Arbeitsstand-Block 2026-08-02 entfernt, §13.5–13.7 (Instandhaltung, AP-Zahlensatz, Regolith-Herleitung inkl. Phase-1-Pacing) aus fünf Nachtrag-Schichten zu einer aktuellen Herleitung zusammengeführt, §4c Harvester/Zweitinstanz, §4b Pfad-C-Hebel, §12 Kanal 1 (Corvan), §15 Fail States/Gnadenfrist (Phase-2-Sol-Basis, geplante Teile markiert), §18.4 Objective-/Credits-Kalibrierungsregeln neu geschrieben; §18.5 Drift-Tabelle entfernt; Tick-Schrittnummern auf `GameTick.php` (1–15) gezogen; INNN-Terminologie durch Kolonieprotokoll/Nexus-Funk ersetzt; §4-Gebäudetabelle, §8b-Missionstabelle, §10-Effektliste auf Config-Stand. GDD 3640 → ~2990 Zeilen, Historie in Git. Neuer Audit-Fund C16: das beschriebene Agrardom-Gate am CC-Levelup existiert im Code nicht.
- Balance: `decay.overcap_factor` 2.0 → 1.5 (GDD §13.1, Audit A4) und Handels-AP `bar.ap_cost_accept` 1 → 2 / `ap_cost_negotiate` 3 → 4 (GDD §13.6 Handlungs-AP, Audit A5) — reine Config-Werte, Tests lesen sie aus der Config; `docs/game-reference.md` nachgezogen.
- Docs: `docs/gdd/techtree.md` auf Config-Stand (Audit C15) — Max-Level-Spalte (Ausbaustufen-System), Agrardom-Gate, Stratege-Zeile aus dem Grid entfernt, Geologie in Phase 2.
- Planung: Audit-Kategorien A + C und die acht Owner-Fragen als „Nächste Woche (ab 2026-09-07)" oben in `ROADMAP.md` aufgenommen (Owner-Entscheidung).
...
- Fix: Techtree-Graph aktualisierte sich nach einem Level-Up-Invest nicht live — abhängige Kenntnisse/Gebäude blieben bis zum Reload als "GESPERRT" angezeigt, obwohl das Server-Gate schon erfüllt war. `TechtreeController::order()` liefert jetzt zusätzlich `phases_update` (Status + Pfeile aller Knoten, dieselbe Gate-Logik wie beim Seitenaufbau), `techtree-view.js::applyPhasesUpdate()` patcht damit den kompletten Graph in-place nach jedem erfolgreichen Invest.
...
- Content/Fix: AP-Typ-Namen (Navigations-AP/Forschungs-AP/Bau-AP/Wirtschafts-AP) aus Spielertext entfernt — AP-Pool ist seit der Konsolidierung ein einziger gemeinsamer Pool, Text vereinheitlicht auf generisches "AP". Techtree-Sidebar für Berater zeigt jetzt eine Beschreibung statt Status/AP-Typ/Einstellungskosten (gehören in den Berater-Screen); Berater-Screen selbst bekam Ressourcen-Chips statt Fließtext, durchgehend ohne "/Tick"-/"/Sol"-Suffix im Chip. Nachzügler gefixt: `KnowledgeEffectDescriptionService`-Effekt-Labels ("Bau-AP-Kosten"/"Navigations-AP-Kosten") hatten die erste Aufräumrunde übersprungen.
- Fix: Kolonie-Sidebar (Hexview) — Gebäude-Detailansicht bekam `required_list` (Voraussetzungen) und Hex-Tiles mit aktivem Harvester bekommen `regolith_remaining`/`regolith_max`, damit die sinkende Ertragskurve (GameTick-Depletion) nicht wie ein Bug wirkt. "Voraussetzungen"/Bild-Fallback für Gebäude ohne Artwork über ein gemeinsames Partial (`partials/required-list-chips.blade.php`) mit dem Techtree-Panel geteilt.
...
- Feat (Branch `feat/hangar-mission-success-chance`, siehe 2026-09-04 für Abschluss): Hangar-Missionen bekommen Erfolgschance + wählbaren Schwierigkeitsgrad statt Garantie-Reward — Plan mit 12 Tasks per Subagent-Driven Development, Tasks 1-10 fertig+reviewed (Migration, Config, `successChanceFor()`, Dispatch-Validierung, Tick-Resolution mit Erfolgs-Roll/Reward-Skalierung/Hard-Fail-Wear, Sol-Report, Lang-Strings, Katalog-Preview), Task 11 (Alpine-Dispatch-Dialog UI) angefangen.

## 2026-09-04

- Feat: Hangar-Missions-Erfolgschance fertiggestellt (Tasks 11-12 + Whole-Branch-Review-Fixes) — Alpine-Dispatch-Dialog zeigt jetzt Schwierigkeits-Chips inkl. Reward-Multiplikator, GDD §8b + `docs/game-reference.md` aktualisiert. Review-Funde behoben: PlaytestBot-Dispatch fehlte `difficulty` (Suite `playtest` war rot, aber von `laravel-feature` ausgeschlossen und dadurch übersehen), Difficulty-Picker wurde bei Missionen ohne Ziel übersprungen (Sofort-Dispatch beim ersten Klick), Comm-Log-Label für `hangar.mission_failed` fehlte, Reward-Multiplikator wurde im Dispatch-Dialog nie angezeigt.
- Refactor: Schwierigkeitsgrad-Identifier von Deutsch auf Englisch umbenannt (`leicht`→`easy`, `schwer`→`hard`; `normal` unverändert) — Config-Keys, DB-Werte, Validierung, Lang-Keys (Werte bleiben deutsch) und `docs/game-reference.md` betroffen. GDD-Prosatext unverändert (nennt die Stufen nur als gewöhnliche deutsche Adjektive, keine Code-Identifier).
...
- Fix: Owner-Korrektur zum vorigen Redesign — Callout-Box bei "Schaltet frei" und die Box um "AP investieren" wieder entfernt, Bullet-Punkte bei Voraussetzungen/Freischaltungen entfernt (nur noch dezente Trennlinien zwischen den Info-Zeilen, sonst reine Typografie/Weißraum, keine Kästen/Einrückungen) — "keine unnötigen Boxen, keine Einrückungen, keine Bullet-Points" (Owner-Playtest-Fund, 4. Iteration).
- Fix: `techtree-view.css` hatte als einzige Asset-Datei im Colony-Layout kein Cache-Busting (`?v=filemtime`) — Browser konnten eine veraltete, bereits behobene CSS-Version zeigen (erklärt vermutlich die scheinbar hartnäckigen Einrückungen). AP-Fortschrittssegmente zusätzlich von einer flex-gestreckten Einzeilen-Leiste zurück auf feste, umbrechende 18px-Kästchen — bei 28+ Segmenten in einer Zeile waren die Tap-Ziele auf Mobile fast untappbar (Owner-Playtest-Fund, 5. Iteration).
...
- Fix: Die 4 "AP übrig"-Onboarding-Hints ("Forschungs-AP", "Bau-AP", "Navigations-AP", "Wirtschafts-AP") sprachen weiterhin von separaten AP-Pools, obwohl seit der AP-Pool-Konsolidierung nur noch ein gemeinsamer Pool existiert — auf "Es sind noch AP übrig" vereinheitlicht (Owner-Playtest-Fund).
- Fix: Protokoll zeigte für `colony.passive_credits` und `colony.stipend_purchased` den rohen Event-Key statt eines übersetzten Textes (fehlende Fälle in `CommLogController::buildDescription()`) — ergänzt. Zusätzlich: AP-/Credits-Beträge im Protokoll (Bau-Investitionen, Berater-Anstellung) waren reiner Text statt der Chip-Optik aus der Resourcebar — neue `<x-amount-chip>`-Komponente, wiederverwendet für alle betroffenen Beschreibungen (Owner-Playtest-Fund).
- Fix: Amount-Chips im Protokoll — additive Beträge (Nexus-Subvention) zeigten unnötig ein "+" als separaten Text davor, Abzüge (Kolonisten-Zulage, Berater-Kosten) gar kein Vorzeichen. Konvention vereinheitlicht: Additiv = kein Vorzeichen, Abzug = Minuszeichen INNERHALB des Chips (Owner-Playtest-Fund).
...
- Feat: health-Kenntnis trägt jetzt additiv zur Seuchenausbruch-Risikoreduktion bei, zusätzlich zur Krankenstation (Owner-Entscheidung 2026-08-27) — beide Quellen summiert gegen denselben Wirkungsdeckel, kein Stack-Ausschluss (die zuvor zitierte GDD-Regel dazu galt nachweislich nur für eine andere, unverwandte Hangar-Mission-Überschneidung). Neuer `config('game.health_plague_risk_reduction_per_lv')`, verdrahtet in `GameTick::rollPlague()`.
...
- Feat: Gebäude-Ausbaustufen-Grundgerüst implementiert (siehe Spec oben) — `max_level`-Korrekturen für 5 Gebäude (housingComplex 6→3, bioFacility/infirmary/bar null→3, sciencelab null→5) in `config/buildings.php`, `tiers`-Config-Arrays + `tier_*`-Sprachdatei-Keys für die benannten Beinamen, neuer `ColonyController::resolveTierLabel()`-Helper zeigt den Beinamen im Kolonie-Hex-View an (Alpine, römische Stufenzahl gem. Spec-Namensmuster). Whole-branch-Review-Fix mitgeliefert: `data/sql/testdata.sqlite.sql` hatte die alten `max_level`-Werte noch stehen (Config→DB-Sync läuft nur über `game:sync-config`, das nichts automatisch aufruft) — Seed korrigiert + Parity-Regressionstest (`BuildingMaxLevelParityTest`) ergänzt, damit dieser Config/DB-Drift künftig auffällt.
...
- Regolith-Startbestand 200→300→340 in zwei Iterationen (siehe GDD §13.7 Nachträge 08-12/08-13), plus zwei PlaytestBot-Bugs gefixt (bioFacility-Priorität, Pfadgebäude-Levelup-Puffer). Ergebnis: `phase2_start_sol` 20-22 durchgehend. PR #246.

## 2026-08-12

- PlaytestBot: Harvester-Zweitinstanz-Pfade (Weg A/B) implementiert (`BotStrategy.php`), inkl. Tile-Occupied-Bugfix bei der Platzierung. PR #244.
- Phase-1-Sol-30-Deadline: neuer vierter Fail-State + Nexus-Eskalation implementiert. Spec: `docs/superpowers/specs/2026-08-12-phase1-sol30-deadline-design.md`. PR #245.

## 2026-08-11

- GDD-Balance-Audit G4: Hangar-Preis 120→95 Rg, alle Pfadgebäude preisgleich (siehe GDD G4). Dev-DB nachsynct (`game:sync-config`, war seit 08-03 nicht mehr gelaufen).
- PlaytestBot: Ring-Cap-Deadlock in `exploreCandidate()` behoben (Bot fror ab Sol 20 komplett ein).

## 2026-08-10

- AP-Pool zusammengelegt (5 Domänen → 1 gemeinsamer Kolonie-Pool), GDD §13.1. `PersonellService` → `AdvisorService` umbenannt/verschoben.
- Whole-Branch-Review-Fixrunde zur AP-Pool-PR: mehrere Domänen-Reste in UI/Docs/Tests nachgezogen (Details siehe PR #240).
- Folgefix: `task_senior_advisors` nach `max_slots` 5→4 unerreichbar — jetzt config-referenziert statt hartcodiert, GDD-Kenntnisse-Matrix nachgezogen.

## 2026-08-06

**§13.7-Regolith-Zahlensatz vollständig gegen die 1-Harvester-Sockel-Baseline neu hergeleitet und freigegeben** (docs/GDD.md, Design-Doku, kein Codepfad). Zielkolonie-Bedarf bleibt stabil bei 1.795 Rg, Sockel-Einnahmen sinken auf 1.032 Rg (12,9 Rg/Sol × 80 Sole, statt der alten 2-Instanzen-Annahme), Sockel-Anteil fällt auf ~57,5 % — Pfad A (`geology`) + Pfad B (`mission_supply_run`) schließen die verbleibende Lücke (18,25 ≥ 14,1 Rg/Sol reif) gemeinsam, ohne Zahlenänderung. Zwei daraus entstandene Owner-Entscheidungen getroffen, beide ohne Spielwert-Änderung: **G6 umformuliert** (Sockel trägt ~57 % allein statt der arithmetisch nicht mehr zutreffenden ~70 %) und **G2-Metrik umgestellt** auf „Anteil am Gesamteinkommen (Sockel + aktiver Pfad-Hebel)" statt „Sockel allein" — löst die entdeckte Kollision mit der 60-%-Instandhaltungs-Obergrenze (Vollausbau wäre sonst bei ~79 % gelandet) und trifft G2s eigenen ~40-%-Zielwert fast exakt. Die vorher vorläufige 247-Cr/Sol-Pfad-C-Zielgröße ist als falsch hergeleitet zurückgezogen (Regolith-Lücke fälschlich in Credits umgerechnet, obwohl Pfad C keinen Regolith-Hebel trägt) — eine echte Credits-Bilanz-über-den-Run fehlt noch, neuer offener Punkt in Anhang A. Der Weg-B-Reparaturkosten-Fehler aus PR #237 (Harvester ist Regolith-Reparatur-exempt) ist im GDD korrigiert (0 Rg + ~14–15 AP statt 1 Rg/SP).

## 2026-08-05

**Harvester-Zweitinstanz-Bezugsquelle implementiert** (§4c, freigegebener Vorschlag von oben, TDD-Umsetzung). Der alte CC-Lv3 + 100-Rg-Weg ist ersetzt: `HarvesterEntitlementService` ist die neue single source of truth ("hat der Spieler die zweite Instanz verdient, und über welchen Weg" — als per-User-Onboarding-Trigger, keine neue Tabelle nötig). **Weg A:** `CorporateContactService` — Orins Angebot ist zustandslos (keine Visits-Tabelle), reine Funktion aus (Kolonie, Tick), Zwei-Ebenen-Würfelwurf (Erscheinen ~alle 15–25 Sole, Angebot bei Erscheinen ~30 %), Preis 400–800 Cr, gated auf CC Lv3 + `instance_count < 2`; neuer Endpoint `POST /colony/corporate-contact/buy-harvester`. **Weg B:** neue Mission `mission_harvester_salvage` (`config/missions.php`, Muster wie `mission_ruin_expedition`), neuer Reward-Typ `harvester_instance` in `GameTick::payMissionRewards()`, Dispatch-Gate `instance_count < max_instances` in `HangarService::dispatchShip()`. `ColonyController::placeBuilding` verlangt jetzt für Instanz 2 CC Lv3 **und** eine erworbene Entitlement (kein Regolith-Kostenpunkt mehr); salvage-Herkunft liefert die Instanz beschädigt (~25 % SP, `config('game.harvester.salvage_arrival_sp_pct')`), Orin-Herkunft voll funktionsfähig. 8 neue/angepasste Testdateien, volle Suite grün (940 Tests). Offen (nicht Teil dieses PRs): `docs/characters/corporate_rep.md` `Game Role`-Update (content-writer), UI-Anbindung (ui-specialist), volle §13.7-Neuherleitung.

**Harvester-Zweitinstanz-Bezugsquelle: UI-Anbindung** (§4c, Folgearbeit zum obigen PR). Orins Angebot (Weg A) bekommt eine eigene Cantina-Fläche: `bar.blade.php` lädt `GET colony/corporate-contact/offer` client-seitig beim Laden (zustandslos, kein Visits-Abgleich nötig) und zeigt bei aktivem Angebot ein Banner mit Portrait/Preis, das den bestehenden `<x-cantina-dialog>`-Kaufdialog öffnet (Muster wie die Merchant-Karte) — Kauf über `POST corporate-contact/buy-harvester`, danach Colony-Ansicht wie gewohnt zum Platzieren. Instanzplatzierung selbst war in `colony-hexgrid.js` bislang nicht mehrinstanzfähig: `harvesterBuilding()`/`doMoveHarvester()` griffen ungeachtet der `instance_id` immer auf den ersten gefundenen Harvester zu und `doMoveHarvester()` schickte gar kein `instance_id`-Feld mit — eine zweite Instanz wäre serverseitig nie unterscheidbar bewegt worden. Behoben: `startHarvesterMove()` erfasst jetzt die konkret angeklickte Instanz, `doMoveHarvester()` sendet `instance_id` mit; neue Aktion `doPlaceHarvesterInstance2()` + Button „Harvester platzieren" auf jedem leeren, erkundeten Regolith-Tile (kein Baumenü-Eintrag, wie von der GDD-Entscheidung gefordert) — die Berechtigungsprüfung bleibt serverseitig (`HarvesterEntitlementService`), das Frontend zeigt nur die vom Server gelieferte Ablehnungsmeldung. Neue `lang/de/colony.php`/`lang/en/colony.php`-Keys für Orins Dialogtexte (TODO-Platzhalter, content-writer-Politur ausstehend, mirrors bereits vorhandenes Muster bei `mission_harvester_salvage_*`).

**Nachgezogen (main):** `CorporateContactController::buyHarvester()` liefert jetzt `credits` im Erfolgsfall (Resourcebar-Live-Sync-Konvention, TDD-Test ergänzt) und ein vorübersetztes `message`-Feld bei Ablehnung (wie `ColonyController::fail()`) statt eines rohen Fehlercodes — clientseitiges Error-Mapping in `bar.blade.php` bleibt als Fallback für unbekannte Codes stehen. Außerdem ein Blade-Component-Parser-Bug behoben: `<x-cantina-dialog>` mit verschachtelten doppelten Anführungszeichen in Attributwerten (`:portrait-src="asset("...")"`) kompilierte lautlos falsch (fehlender `startComponent`-Aufruf, Laufzeitfehler erst beim Rendern) — auf vorberechnete `@php`-Variablen umgestellt, wie beim bestehenden Merchant-Dialog. 950 Tests grün, Pint sauber. Kein Dev-Server/Browser-Test durchgeführt.

**Harvester-Zweitinstanz-Bezugsquelle entworfen und freigegeben** (§4c, docs/GDD.md — Design, noch keine Implementierung). Bestätigter Gap: Backend akzeptiert `instance_id=2` für den Harvester, aber `ColonyController::getBuildableBuildings()` schließt ihn komplett vom Baumenü aus — kein Spielerweg zur zweiten Instanz. Nach drei Owner-Korrekturrunden (Baumenü-Fix abgelehnt: keine logische Wiederholbarkeitsgrenze, Harvester ist Schwerausrüstung, keine Kolonie-Infrastruktur; Nexus-Direktimport abgelehnt: zu planbar/garantiert) steht die freigegebene Lösung: **§13.7-Sockel-Baseline auf 1 Harvester-Instanz umgestellt** (1 Instanz muss für einen Run reichen, 2. ist optionaler, nicht garantierter Bonus). Bezugswege: **Weg A** — Orin (`corporate_rep`, bestehender Cantina-Charakter mit Nexus-Extraktionsrechte-Background) verkauft ein Harvester-Modul für 400–800 Cr bei seltenem, nicht planbarem Erscheinen (~40–60 % Trefferquote pro Run). **Weg B** — neue Bergungsmission `mission_harvester_salvage` auf einer `ruin_tile`, kostenlos aber beschädigt ankommend (Reparaturpfad §7). Offen: volle §13.7-Neuherleitung auf die neue Baseline (eigener Termin), TDD-Umsetzung, Orins `Game Role`-Content-Update, `ruin_tile`-Spawnrate im Code unverifiziert.

**Pfad-C-Regolith-Hebel neu gedacht und freigegeben** (§4b/§12, docs/GDD.md — Design, noch keine Implementierung). ROADMAP-Punkt „Pfad-C-Regolith-Hebel neu denken" (Stufe 1b) beantwortet: kein Regolith-Hebel mehr (Regolith läuft pfad-unabhängig über den Harvester-Sockel), stattdessen ein Credits-Hebel über die Cantina. **Corvan (Reisender Händler, `merchant.md`/`MerchantService`) wird zur zentralen, häufigeren Cantina-Handelsfigur** (~5–8 statt 10–15 Sole Erscheinungsintervall) — Fund dabei: sein Charakterblatt trug bereits `Game Role: bar_trade`, nur nie technisch angebunden. Zwei Angebotsebenen pro Besuch: häufiges Alltagsgeschäft (Standard-Rohstoffe gegen Credits, inkl. neuem Organika-Verkauf) + weiterhin seltenes kuratiertes Sonderinventar (unverändert). `BarService`-Gäste (Dax, Voss, …) verlieren Credits-Handel komplett, nur noch Tauschhandel. Zielgröße gegen die neue 1-Harvester-Sockel-Baseline neu gerechnet: ~247 Cr/Sol (vorher 145 Cr/Sol). Offen: TDD-Umsetzung (Zusammenführung `BarService`/`MerchantService`-Spawn-Logik), Corvans Bio-Update (`content-writer`), gemeinsame §13.7-Neuherleitung mit dem Harvester-Thread.

**Pfad-C-Hebel implementiert: Corvan als zentrale Handelsfigur der Cantina** (§4b/§12, TDD). `BarService`s anonyme Gästerotation (Dax, Voss, …) verliert Credits-Handel vollständig — nur noch Tauschhandel; die alte Zahlungsfähigkeits-Cap-Logik (`buildOffer()`) wandert als `BarService::buildCorvanBuyOffer()` zu Corvan, jetzt ohne Barter-Fallback (unerschwingliches Angebot entfällt einfach). `MerchantService::spawnVisit()` generiert bei jedem Corvan-Besuch zusätzlich zum unveränderten Sonderinventar Corvans Alltagsgeschäft: 2–3 Organika-Verkaufslose (15–25 Einheiten, 35 Cr/Einheit, Reserve-Untergrenze 2× `food_need`) plus einen optionalen Kaufoffer — persistiert als `bar_offers`-Zeilen mit neuem `visit_id`-FK (Migration, kein Backfill nötig), damit Annahme/Verhandlung/AP-Abbuchung ohne Parallelpipeline durchlaufen. Erscheinungsintervall `game.merchant.interval_min/max` 10–15 → 5–8 Sole; `MerchantService::shouldSpawn()` dabei von einer komponierten Cooldown+Wahrscheinlichkeits-Formel (realer Erwartungswert lag weit über dem konfigurierten Maximum) auf einen deterministischen Ziel-Gap innerhalb `[interval_min, interval_max]` umgestellt. Reserve-Untergrenze zusätzlich in `BarService::acceptOffer()` scharf geschaltet (nicht nur bei Generierung), da über die 2-Sol-Visit-Dauer mehrere Lose angenommen werden können. Neue `ResourcesService::foodNeed()`-Methode als Single Source of Truth, von `GameTick` und dem Reserve-Check gemeinsam genutzt. Neuer Lang-Key `colony.bar_offer_reserve_floor`. `spawnVisit()` läuft jetzt atomar in einer `DB::transaction()` (Visit + Items + Commodity-Offers zusammen). PR #238 (offen, nicht gemerged).

**Cantina-UI:** Corvans Alltagsgeschäft-Angebote sind jetzt visuell von anonymen Gast-Angeboten unterschieden (eigenes Portrait/Name, Gold-Badge statt anonymem Gast-Icon). Nebenbei behobener Bug: Offer-Hotspots nutzten nur 2 von 5 vorgesehenen Cantina-Slots, wodurch mehr als 2 gleichzeitige Angebote sich überlappten.
...
Testlauf der Design-System-Verdrahtung: Lobby-Screen (`resources/views/lobby/index.blade.php`) komplett auf Design-System-Tokens umgestellt (Farben, Typografie, Spacing, Card-Stil mit Akzent-Rahmen statt Schatten) statt Bootstrap-Pico-Grau. Bestätigt, dass der `ui-specialist`-Agent das Design System tatsächlich konsultiert und nicht nur Farben frei erfindet.
...
- Fix (eigentliche Ursache, per Playwright gefunden): `:class="[{ 'open': ... }, 'foo']"` (Array mit Objekt drin) löst Alpine.js v3 nicht wie Vue auf — das Objekt wird zu `"[object Object]"` statt zu Klassennamen, `.open` fehlte, Dialog blieb bei `opacity:0`. Fix: reine Objekt-Syntax mit computed key (`{ open: ..., ['cantina-modal--' + type]: true }`). Per Playwright-Klicktest verifiziert (computed `opacity: 1`, Screenshot zeigt Dialog korrekt inkl. Portrait + Border-Akzent).
...
- **Techtree-Ablehnungen nannten keinen Grund.** `POST /techtree/{type}/{id}/order` antwortete immer `200 {success:false}` — weder Spieler noch Client konnten "zu wenig AP" von "Kommandozentrale zu niedrig" unterscheiden. Neu: **422** mit `{error:'<code>', message:'<Text>'}` wie bei Colony/Hangar. Die Services benennen den Grund über `levelupBlocker()` / `investBlocker()`; `levelup()` und `_invest()` delegieren dorthin, statt die Prüfungen ein zweites Mal zu führen (`ResearchService::levelup` entfällt dadurch ganz, sein Kenntnis-Gate lebt jetzt im Blocker).
- **Zwei erreichbare 500er im Techtree gefunden und behoben.** `PersonellService` erbt nicht von `AbstractTechnologyService` und hat kein `invest()`/`levelup()`, wurde vom Controller aber auf `{type}=personell` gemappt: `POST /techtree/personell/35/order` starb mit "call to undefined method". `{type}` ist in der Route unbeschränkt, also von außen erreichbar. Berater werden über den Berater-Screen eingestellt, nicht über den Techtree — `personell` ist jetzt kein gültiger Techtree-Typ mehr (422). Ebenso warf `action()` bei unbekanntem Typ `InvalidArgumentException` → 500, jetzt 404.
- **Techtree ignorierte `game.bypass.*` beim AP-Gate.** Mit Dev-Mode war das ganze Spiel frei klickbar — außer dem Techtree, der weiter AP verlangte. `_invest` respektiert jetzt `ap_checks` (Prüfung *und* Locking, wie `ColonyTileService`). Bewusst **nicht** bypassbar bleibt die `ap_spend`-Schwelle: sonst ließen sich Stufen ganz ohne Investition abschließen. Ressourcen- und Supply-Prüfung respektierten die Flags bereits.

Tests: 715 (12 neu), unverändert 1 vorbestehender Fehler (`TrustServiceTest::test_get_band_unknown_locale`). Bekannt offen: der Techtree-Screen hat keine Fehleranzeige — ein abgelehntes Investment zeigt dem Spieler weiterhin nichts (`techtree-view.js` bricht bei `!res.ok` still ab). Der Text steht jetzt in der Antwort bereit; ihn anzuzeigen ist UI-Arbeit für einen eigenen Durchgang.
...
- **Schiffs-Verschleiß implementiert** (GDD §7). Neuer `GameTick`-Schritt `processHangarMissions()`: zieht `wear_per_sol` (config/ships.php: Drohne 1,5 / Frachter 1,0 / Korvette 0,75) pro Tick von jedem dispatchten Schiff ab; SP ≤ 0 → Mission bricht ab (kein Ertrag, Schiff kehrt flugunfähig zurück), sonst Completion bei `dispatch_tick + 2×sol_distance` mit Reward-Auszahlung (Credits/Ressourcen/Trust-Events/Tile-Reveal/Tiefenscan/Research-AP/Loot-Table, deterministisch aus `runs.rng_seed`). Beide Ereignisse protokolliert (`colony_log`, Sol-Report).
- **Migration:** `colony_hangar_missions.target` (json) für spielerseitig gewählte Missionsziele. `mission_nav_ap_per_sol`/`mission_organika_per_sol` von `config/game.php` nach `config/missions.php` verschoben; `trust.events.encounter_won` ergänzt (fehlte seit Galaxie-Streichung).
...
- **Fix (Owner-Test):** Missionskatalog prüft jetzt vorab Navigations-AP und Organika gegen den verfügbaren Bestand — Zustände `missing_ap`/`missing_organika` ergänzen `missing_knowledge`/`missing_target`; Karte ausgegraut, Button gesperrt, Hinweistext mit Ist/Soll ("Nicht genug Navigations-AP (0/2 verfügbar)"). Vorher konnte man erst beim Klick auf "Mission starten" scheitern.
