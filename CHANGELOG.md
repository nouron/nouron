...
- **Pre-commit Lint-Hooks** — `.githooks/pre-commit` lintet vor jedem Commit: PHP via Laravel Pint (Auto-Fix), JS/CSS via Prettier (Auto-Fix), Blade via Prettier `--check` (blockt, kein Auto-Write — Plugin zu aggressiv für Alpine-Templates). Aktivierung pro Clone: `npm install && git config core.hooksPath .githooks`. Einmalige Baselines: Prettier über `public/js`+`public/css`, Pint über `app/tests/config/lang/routes/tools` (`database/migrations` dauerhaft ausgeschlossen). Configs: `.prettierrc.json`, `.prettierignore`, `pint.json`.
