...
**Eingeschränkter Bash-Zugriff für `project-manager` und `ui-specialist`** (Agenten-Konfiguration). Beide Agenten hatten kein Bash, konnten aber auch nicht delegieren — Subagenten haben kein `Agent`-Tool, sie geben Befehle nur an den Hauptthread zurück. Konkrete Folge: `project-manager` konnte für CHANGELOG-Einträge und PR-Beschreibungen kein `git log` lesen (Inhalte mussten gefüttert werden), `ui-specialist` konnte das für Blade verpflichtende `npx prettier --write` nicht ausführen, obwohl der Pre-Commit-Hook Blade nur prüft und nicht auto-formatiert. Statt vollem Bash gibt es jetzt ein per-Agent-Profil über einen `PreToolUse`-Hook (`.claude/hooks/bash-allowlist.sh`): `git-readonly` erlaubt nur lesende git-Subkommandos, `frontend` zusätzlich Prettier und `npm run build|install|ci`. Der Guard zerlegt verkettete Befehle (`&&`, `||`, `;`, `|`, Zeilenumbruch) und prüft jedes Segment einzeln; Kommando-Substitution, Umleitungen, Hintergrundausführung und Env-Zuweisungen vor dem Befehl (`GIT_PAGER=sh git log`) sind blockiert, ebenso globale git-Optionen wie `-c`/`--exec-path`, die externe Programme starten können. Testabdeckung TDD-first: 60 Fälle inkl. Umgehungsversuchen in `.claude/hooks/tests/bash-allowlist.test.sh`. Neue erlaubte Befehle gehören in die Allowlist plus Testfall, nicht als Ausnahme in den Agent-Prompt.
