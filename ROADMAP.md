...
- [ ] **A39 Sciencelab-Kenntnis-Rabatt verstärken** (Nebenbefund A37, 2026-09-13) — Owner-Entscheidung: `sciencelab.knowledge_ap_cost_reduction_per_lv` (`config/buildings.php`, aktuell `[4=>3, 5=>2]`, nur 5% kumulativ, wirkt erst ab Lv4) soll verstärkt werden UND schon ab Lv2 greifen. Vom game-designer kalibrierter Vorschlag: `[2=>6, 3=>7, 4=>8, 5=>9]` (kumulativ 6/13/21/30% bei Lv2-5, bleibt unter dem 50%-Floor `project_min_cost_factor`) — senkt die 540 AP für `task_research_lead` bei durchgehend Sciencelab Lv5 auf ~378 AP. Noch nicht umgesetzt (TDD: `ResearchServiceKnowledgeDiscountTest` um Lv2/Lv3-Fälle erweitern, `docs/game-reference.md` Kenntnis-Tabelle nachziehen). Nach A39 erneut Playtest-Batch fahren, um zu prüfen ob `task_research_lead` dadurch erreichbar wird. — Klein
