...
- [ ] **A13 Handel/Konsul-Komplex** (§4, §10, §12, §13.3; Ausgangspunkt: Handelsposten „Konsul-Effizienz") — wuchs 2026-09-20 nach zwei game-designer-Reviews zu einem Komplex aus. **Owner-Entscheidungen:** „Konsul-Effizienz" wird gestrichen; Kenntnis `trade` senkt KEINE Bau-AP mehr (thematisch falsch); der Konsul bekommt zusätzlich zu AP aktive Bonus-Effekte (Schiffsverhandlung, Marktbericht; „Wunschangebot" nur Ideenpool); Verhandeln soll sich „mal lohnen, mal nicht"; Handelsposten Stufe III wirkt auf den Nexus-Direktimport-Preis; **Verständlichkeit für den Spieler hat hohe Priorität** (Angebotsdialog zeigt Basis, Vorteil je Quelle, Chance und Ergebnis); Annehmen und Verhandeln kosten beide 2 AP (Owner-Idee, Designer bestätigt und neu kalibriert). Konzept im GDD §12 „Handelsvorteil" (Konzeptstand).
  ...
  - [ ] **P4** Nexus-Kanal / Handelsposten Stufe III (Direktimport-Preis, `corporate_contact` → `nexus`, Vorschau-Pfad) — M — **weitgehend fertig auf Branch `wip/a13-p4-nexus`** (Tests grün); offen: Browser-Verifikation, volle Suite, Pint/Larastan/Prettier
  - [ ] **P7/P8** Nachzug `docs/game-reference.md` (u.a. `game.hangar.consul_ap_discount`, `game.merchant.forecast_*`, `game.bar.trade_terms`, Verhandeln-Werte, `trade`-Effekte), Config-Kommentare, GDD-Vermerke „Konzeptstand" → implementiert; content-writer-Pass über alle `TODO(content-writer)`-Texte (P2a–P6, u.a. Fehlschlag-Meldung grammatisch für Plural-Ressourcen); Bot-Batch 8–10 Seeds (Corvans Kaufangebote werden nach P2a etwas häufiger weggelassen) — S
...
- [ ] **T8 Realistische Test-/Dev-Fixture (Supply)** (Fund 2026-09-20) — `migrate:fresh --seed` liefert für Bart einen unrealistischen Sol-5-Stand mit **Supply −34 / 18**: `data/sql/testdata.sqlite.sql` (Zeilen ~107–113 `colony_buildings`, ~190 `user_resources`) setzt u.a. Krankenstation Lv3 (30 Supply), Sciencelab, 2 Hangars und Harvester (52 belegt) bei fest hinterlegter Kapazität 18 — im Normalspiel unerreichbar, denn Bauen/Ausbauen ist bei zu wenig freiem Supply gesperrt (`ColonyController::placeBuilding`, `AbstractTechnologyService::checkRequiredSupplyByEntityId`). Auch nach einem Tick (Kapazität neu berechnet) bliebe es bei ca. −26. Zu tun: (1) Fixture-Stand auf einen im Spiel tatsächlich erreichbaren Zustand bringen (Supply ≥ 0, Gebäude/Kapazität konsistent) — Achtung, die Fixture trägt die ganze Testsuite, Auswirkung auf bestehende Tests vorher prüfen; (2) die hartcodierten `supply`/`regolith`-Werte in den `game:reset-player`-Szenarien (`pre-phase2`, `phase2`, `near-fail-trust`, `near-deadline`, `objectives-done`) prüfen und nachziehen (siehe auch Eintrag „`ResetPlayer.php`: hartcodierte …" weiter unten); (3) optional: `ValidateColony`-Check, dass kein Szenario/keine Fixture Supply < 0 erzeugt. Workaround bis dahin: `php artisan game:reset-player bart --yes --scenario=fresh` (liefert Sol 1 mit 13 von 15 Supply frei, verifiziert). — Klein/Mittel
