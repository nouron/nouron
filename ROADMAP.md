...
- [ ] **A34 Nexus-Schulden-Threshold neu kalibrieren (Playtest)** (nach A21) — `nexus_debt_fail_threshold` (aktuell 12000) gegen einen Playtest-Batch (mind. 6-10 Seeds) prüfen, jetzt wo Upkeep-Defizit zusätzlich zu Startkredit + Schiffskauf-Kredit einzahlt — könnte den Fail-State früher/häufiger auslösen als vor A21. Kein Wert raten, erst nach echten Lauf-Daten anpassen. — Klein, spät (nach nächstem Playtest-Batch)
...
- [ ] **A35 Cantina-Begegnungspool: mechanisches Grundgerüst** (nach A22, §12) — Ersatz-Hebel für den Pfad-C-Credits-Hebel (Owner-Entscheidung 2026-09-09, GDD §4b/§12): ein gemeinsamer Ereignis-Slot mit drei Ausgängen — Wette (Zara, Einsatz gegen Chance-Auszahlung), Auktion (Voss, Einmalzahlung Ressource→Credits), Kurzzeit-Kontrakt (generisch, Credits/Sol für festes Fenster). Alle drei skalieren über Bar-Level, NICHT über Konsul-Rang (sonst wiederholt sich der A22-Fehler eine Ebene tiefer). Ein Roll statt drei unabhängige Spawn-Checks (sonst Häufungsrisiko mit Corvan/Gästerotation). TDD: Test zuerst je Ausgang + Frequenz-/Skalierungslogik. Charakterbindung Zara/Voss braucht Abstimmung mit content-writer (Dialog/Lore), volle Zuordnung der übrigen 12 Cantina-Figuren ist eigener Folge-Task (siehe A36), kein Blocker hier. — Mittel/Groß
- [ ] **A36 Cantina-Begegnungspool: volle Charakter-Zuordnung** (nach A35, content-writer) — allen übrigen Cantina-Figuren eigene Begegnungen zuordnen (Lore/Dialog, weitere Ressourcen-/Story-Kategorien, ggf. Missions-Trigger analog `project_bar_mission_trigger_idea`). Leitplanke beachten: nicht jede Figur bekommt automatisch ein Credits-/Ressourcen-Event — Pfad-C-Knappheitsordnung (§3) darf nicht aufgeweicht werden (z.B. kein zweiter planbarer Regolith-Kanal über eine künftige Figur wie Fen). — Groß, spät (nach A35)
