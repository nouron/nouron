...
- [ ] **A37 PlaytestBot gewinnt keinen einzigen Run** (Nebenbefund A34, 2026-09-13) — 8-Seed-Batch (`default`-Profil, inkl. Berater-Beförderung): alle 8 Runs `fail_reason=time_limit`, 0-1/3 Ziele erreicht. Eigenständiges, größeres Balance-Problem — braucht eigene Analyse (welches Ziel scheitert warum, AP-/Ressourcen-Engpass oder Bot-Strategie-Schwäche), nicht Teil von A34/A35/A36. — Mittel/Groß
  - **Teilbefund (2026-09-13):** `task_research_lead` (3 Kenntnisse auf Lv5, `RunProgressService::updateResearchLead()`) ist im Bot strukturell unerreichbar. `RunReport` erfasst jetzt Kenntnis-Level pro Sol (vorher gar nicht getrackt, `RunReportResearchSnapshotTest`). Ein voller 99-Sol-Lauf (Seed 1001) zeigte: nur 1 von 7 Kenntnissen je angefasst (`knowledge_construction`, Lv4 von 5 erreicht), die anderen 6 blieben bei Lv0 — `BotStrategy::researchCandidate()` wählt immer die niedrigste noch offene Kenntnis-ID und bleibt stur dabei, bis sie fertig ist (Regel steht zudem erst auf Rang 12/15 der Prioritätsliste). Kosten: 180 AP kumulativ bis Lv5 je Kenntnis (`config/knowledge.php → levelup_costs`), macht 540 AP allein für `task_research_lead`, zusätzlich zu allem Bau-/Reparatur-/Missionsaufwand aus demselben AP-Pool (`ap.base`=12/Sol). In diesem Lauf flossen nur 128 AP in Forschung über 99 Sole. Unklar ob primär Bot-Strategie-Artefakt (analog A34/promote_advisor) oder echte AP-Ökonomie-Lücke — game-designer-Einschätzung angefragt (2026-09-13), Ergebnis noch offen.
