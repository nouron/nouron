...
- [ ] **A40 Cantina-Barkeeper** (Konzeptstand, GDD-Design-Intent siehe `docs/GDD.md` §12 Kanal 1 „Bar/Cantina") — Dauer-Charakter (immer verfügbar, im Gegensatz zu Corvan/zufälligen Begegnungen), Dialog-Bonus-Mechanik: gewährt AP-Bonus zu Kenntnis-Investitionen bei häufigem Austausch. Details/Formel noch offen, game-designer muss Detailplan erst ausarbeiten (TDD-Pflicht sobald Implementierung startet, siehe CLAUDE.md). Folgeaufgabe (separat, nicht Teil des Kernfeatures): Frontend-Anzeige, welcher AP-Anteil vom Barkeeper stammt (Badge/Symbol). — Mittel
- [ ] **A41 Cantina-Charakter-Anliegen bei zufälligen Begegnungen** (Konzeptstand, GDD-Design-Intent siehe `docs/GDD.md` §12 Kanal 1 „Bar/Cantina") — Erweiterung des bestehenden Begegnungspools (Zara/Voss/Kontrakt bleiben bestehen, siehe A35/A36): neue Anliegen-Struktur, Charakter hat Bitte, Spieler hilft (AP-Kosten), erhält variable Belohnung (Ressourcen, Credits, AP-Boost, Baukosten-Rabatt, Zusatzschiff-Kauf). Owner will breite Belohnungs-Vielfalt; vollständige Anliegen-Liste und Detailausarbeitung mit game-designer noch offen, vor Implementierung nötig (TDD-Pflicht sobald Implementierung startet). — Mittel/Groß
- [ ] **A42 Deva/Lenn (information-Rolle) + Charakter-Kodex** (Konzeptstand, GDD-Design-Intent siehe `docs/GDD.md` §12 Kanal 1 „Bar/Cantina", A36-Folge-Task) — zwei Teile, gemeinsam finalisiert 2026-09-16:
  - **Deva & Lenn taktische Information:** wiederholbare Begegnung mit Vier-Ausgänge-Pool je Figur — zwei mechanische Ausgänge (Deva: Drill-Reaktionspuffer für nächste Instabilität/Seuche + Kenntnis-Fortschrittsschub Geologie/Gesundheit; Lenn: Navigation-Rabatt nächste Erkundung + Kenntnis-Fortschrittsschub Kartografie), je einmal pro Run nutzbar (Deckel pro Ausgangs-Typ, nicht pro Figur — löst Wiederholbarkeit ohne Balance-Loop), plus zwei rein erzählerische Ausgänge ohne Spielwirkung, unbegrenzt wiederholbar. Owner-Wunsch: mehrmaliges Antreffen mit Dialog-/Outcome-Varianz statt Einmal-Effekt.
  - **Charakter-Kodex (Meta-Ebene, ganzer Cantina-Cast):** rein narrative Lore-Einträge je Figur, KEINE Spielmechanik-Wirkung, run-übergreifend user-persistent (`user_preferences`-Muster). Freischaltung reused bestehende `game_role`-Interaktionen als Trigger (information: Ausgangs-Pool-Begegnung; bar_trade: abgeschlossener personalisierter Tausch; dedicated: eigene Mechanik; story_hook: die Begegnung selbst; permanent/Tomas: Cantina-Ausbaustufen-Meilensteine statt Begegnung). Owner-Entscheidung: annehmen, kein mechanischer Bonus, explizit als zweiter Varianz-Hebel gewünscht.
  - Offen vor Implementierung: exakte Formelwerte (Puffer-Stärke, Fortschrittsschub, Rabatt-Höhe) → `config/game.php`/`config/knowledge.php`; Kodex-Textmenge + Texte → `content-writer`; Persistenz-Schema → `db-migration-agent`. TDD-Pflicht sobald Implementierung startet. — Mittel/Groß
